...
class MessagingSecurityTest {
    ...
    @Test
    void missionNotificationWithoutInternalTokenIsUnauthorized() throws Exception {
        mockMvc.perform(post("/api/notifications/missions/published")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"missionId":1,"companyId":10,"title":"Build API"}
                                """))
                .andExpect(status().isUnauthorized());
    }

    @Test
    void missionNotificationWithInternalTokenIsAllowed() throws Exception {
        mockMvc.perform(post("/api/notifications/missions/published")
