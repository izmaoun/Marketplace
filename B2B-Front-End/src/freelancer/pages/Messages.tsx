...
import {
  ...
  type TemporalFilterValue,
} from "../../shared/temporalFilters";
import {
  createConversation,
  createMediaReadUrl,
...
export function Messages() {
  ...
  const [periodFilter, setPeriodFilter] = useState<TemporalFilterValue>("recent");
  ...
  const loadMessagingData = async () => {
    ...
    try {
      const range = resolveTemporalRange(periodFilter, customFrom, customTo);
      const [conversationList, missions] = await Promise.all([
        listConversations(range),
      ...
      <div className={`min-w-0 flex-col border-r border-slate-100 bg-slate-50/50 ${activeConversation ? "hidden md:flex" : "flex"}`}>
        <div className="p-4 border-b border-slate-100 bg-white">
          ...
          <div className="mt-3 grid gap-2">
            <select
              value={periodFilter}
              onChange={(event) => setPeriodFilter(event.target.value as TemporalFilterValue)}
              className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500/50"
            >
              {temporalFilterOptions.map((option) => (
            ...
            {periodFilter === "custom" && (
              <div className="grid grid-cols-2 gap-2">
                <input
                  ...
                  className="min-w-0 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500/50"
                />
                <input
                  type="date"
                  value={customTo}
