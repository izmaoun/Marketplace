...
export function Missions() {
  ...
  const requestedMissionId = Number(searchParams.get("missionId"));
  const lastRequestedMissionReloadRef = useRef<number | null>(null);
  ...
  const [status, setStatus] = useState<Mission["status"]>("PUBLIEE");
  ...
  useEffect(() => {
    if (!Number.isFinite(requestedMissionId)) return;

    const requestedMission = missions.find((mission) => mission.id === requestedMissionId);
    if (requestedMission) {
      setSelectedMission(requestedMission);
      return;
    }

    if (!isLoading && lastRequestedMissionReloadRef.current !== requestedMissionId) {
      lastRequestedMissionReloadRef.current = requestedMissionId;
      void loadMissions();
    }
  }, [isLoading, missions, requestedMissionId]);
  ...
  const loadMissions = async () => {
    ...
    try {
      ...
      const requestedMission = Number.isFinite(requestedMissionId)
        ? data.find((mission) => mission.id === requestedMissionId)
        : undefined;
      setMissions(data);
      setSelectedMission((current) => requestedMission ?? current ?? data[0] ?? null);
  ...
  const canApply = selectedMission?.status === undefined || selectedMission.status === "PUBLIEE";
    ...
    <div className="flex h-full w-full relative">
      <div className={`flex-1 flex flex-col transition-all duration-300 ${selectedMission ? "lg:mr-[400px] xl:mr-[500px]" : ""}`}>
        <div className="bg-white p-4 rounded-2xl shadow-sm mb-6 flex flex-wrap gap-3 items-center">
          ...
          <select
            value={status}
            onChange={(event) => setStatus(event.target.value as Mission["status"])}
            className="bg-slate-50 border border-slate-100 text-sm rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer"
          >
              ...
              >
                <div className="flex justify-between items-start mb-3">
                  ...
                  <span className="bg-emerald-50 text-emerald-600 text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-wide">
                    {missionStatusLabels[mission.status ?? "PUBLIEE"] ?? mission.status ?? "PUBLIEE"}
                  </span>
                </div>
        ...
        <>
          ...
          <div className="fixed inset-y-0 right-0 w-full md:w-[400px] xl:w-[500px] bg-white shadow-2xl z-50 flex flex-col border-l border-slate-100 animate-in slide-in-from-right duration-300 lg:absolute lg:h-full lg:shadow-[-10px_0_30px_rgba(0,0,0,0.05)]">
            ...
            <div className="shrink-0 p-6 border-t border-slate-100 bg-white space-y-3">
              ...
              >
                <Briefcase size={18} />
                {isApplying ? "Envoi..." : canApply ? "Postuler directement" : "Mission non ouverte"}
              </button>
            </div>
          </div>
        </>
