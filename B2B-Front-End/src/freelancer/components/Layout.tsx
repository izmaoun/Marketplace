...
import {
  ...
  type Conversation,
  type Mission,
  type MissionPublishedNotification,
...
export function Layout() {
  ...
  const [liveMissionNotifications, setLiveMissionNotifications] = useState<MissionPublishedNotification[]>([]);
  const missionNotificationSocketRef = useRef<MessagingSocket | null>(null);
  ...
  useEffect(() => {
    ...
    socket.connectMissionNotifications((notification) => {
      setLiveMissionNotifications((current) => [
        notification,
        ...current.filter((item) => item.missionId !== notification.missionId),
      ].slice(0, 5));
      setIsNotificationsOpen(true);
    });
    missionNotificationSocketRef.current = socket;

    return () => {
      missionNotificationSocketRef.current?.disconnect();
      missionNotificationSocketRef.current = null;
    };
  }, []);
  ...
  const notifications = useMemo<NotificationItem[]>(() => {
    const liveMissionItems = liveMissionNotifications.map((notification) => ({
      id: `live-mission-${notification.missionId}`,
      kind: "mission" as const,
      title: "Nouvelle mission",
      detail: notification.title
        ? `Nouvelle mission publiee : ${notification.title}.`
        : "Une nouvelle mission vient d'etre publiee.",
      createdAt: notification.publishedAt,
      to: `/freelancer/missions?missionId=${notification.missionId}`,
    }));
    ...
    const missionItems = missions.slice(0, 3).map((mission) => {
      ...
      return {
        ...
        to: `/freelancer/missions?missionId=${mission.id}`,
      };
    });
    ...
    return [...liveMissionItems, ...acceptedItems, ...messageItems, ...missionItems]
      .sort((a, b) => new Date(b.createdAt ?? 0).getTime() - new Date(a.createdAt ?? 0).getTime())
      .slice(0, 5);
  }, [applications, companiesById, conversations, liveMissionNotifications, missions]);
