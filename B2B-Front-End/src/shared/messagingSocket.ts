...
import type { ChatMessage, MissionPublishedNotification } from "./services";
...
type MissionNotificationHandler = (notification: MissionPublishedNotification) => void;
...
export class MessagingSocket {
  ...
  connectMissionNotifications(onNotification: MissionNotificationHandler) {
    this.disconnect();
    ...
    const client = new Client({
      ...
      debug: () => undefined,
      onConnect: () => {
        this.subscription = client.subscribe("/topic/freelancers/missions", (frame: IMessage) => {
          onNotification(JSON.parse(frame.body) as MissionPublishedNotification);
        });
      },
    });
