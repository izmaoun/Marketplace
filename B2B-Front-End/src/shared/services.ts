...
import type { TemporalRange } from "./temporalFilters";
...
export type MissionPublishedNotification = {
  missionId: number;
  companyId: number;
  title: string;
  publishedAt?: string;
...
export function getApplicationCountForAdmin() {
  return apiFetch<number>("/api/admin/v1/applications/count");
}
...
export function listConversations(params: TemporalRange = {}) {
  const searchParams = new URLSearchParams();
  if (params.from) searchParams.set("from", params.from);
  if (params.to) searchParams.set("to", params.to);
  const suffix = searchParams.toString();
  return apiFetch<Conversation[]>(`/api/messaging/v1/conversations${suffix ? `?${suffix}` : ""}`);
}
...
export function getConversationMessages(conversationId: string, params: TemporalRange = {}) {
  const searchParams = new URLSearchParams();
  if (params.from) searchParams.set("from", params.from);
  if (params.to) searchParams.set("to", params.to);
  const suffix = searchParams.toString();
