...
export default function App() {
  ...
  const stats = useMemo(
    ...
    [applicationCount, auditLogs.length, companies, freelancers, missions.length]
  );
  ...
  const loadAdminSpace = async () => {
    ...
    const [profileResult, companiesResult, freelancersResult, missionsResult, applicationsResult, auditResult] = await Promise.allSettled([
      getAdminProfile(),
    ...
    if (companiesResult.status === "fulfilled") {
      ...
      setCompanyMissions(companyMissionMap(resolvedCompanies));
    ...
    const embeddedApplicationCount = totalCompanyApplications(resolvedCompanies);
    if (applicationsResult.status === "fulfilled") {
      setApplicationCount(Math.max(applicationsResult.value, embeddedApplicationCount));
    } else {
      setApplicationCount(embeddedApplicationCount);
      if (embeddedApplicationCount === 0) {
        errors.push(getAdminErrorMessage(applicationsResult.reason, "Candidatures indisponibles."));
      }
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-6 gap-6">
        ...
        <Stat icon={Briefcase} label="Candidatures" value={stats.applications} />
            ...
            <tbody className="divide-y divide-border">
              {filteredCompanies.length === 0 ? (
                ...
                filteredCompanies.map((company) => {
                  ...
                  const companyMissions = (company.missions?.length ? company.missions : missions.filter((mission) => mission.companyId === company.id));
...
async function resolveAdminMissions(primaryMissions: AdminMission[], companies: AdminCompany[]) {
  ...
  primaryMissions.forEach((mission) => missionsById.set(mission.id, mission));
  companies.forEach((company) => (company.missions ?? []).forEach((mission) => missionsById.set(mission.id, mission)));

  if (missionsById.size > 0 || companies.length === 0) return Array.from(missionsById.values());
...
function companyMissionMap(companies: AdminCompany[]) {
  return Object.fromEntries(
    companies
      .filter((company) => (company.missions ?? []).length > 0)
      .map((company) => [company.id, company.missions ?? []])
  );
}

function totalCompanyApplications(companies: AdminCompany[]) {
  return companies.reduce((total, company) => total + (company.applicationCount ?? 0), 0);
}
