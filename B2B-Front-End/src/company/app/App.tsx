...
function CompanyMessagesView({
  ...
  const loadMessagingData = async () => {
    ...
    try {
      const conversationList = await listConversations(resolveTemporalRange(periodFilter, customFrom, customTo));
              ...
              {periodFilter === "custom" && (
                <div className="grid grid-cols-2 gap-2">
                  <input
                    ...
                    className="min-w-0 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10"
                  />
                  <input
                    type="date"
                    value={customTo}
